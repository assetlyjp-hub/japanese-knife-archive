...
引数:
  --type  : 記事カテゴリ (type/steel/budget/use/comparison/brand/guide)
...
def generate_article(client: anthropic.Anthropic, keyword: dict, lang: str,
                     knives_data: list, steels_data: list, brands_data: list) -> str:
    """
    Claude APIを呼び出して記事を生成する。
    包丁・鋼材・ブランドデータをコンテキストとして渡す。
    """
    # 言語に応じたプロンプト
    if lang == "ja":
        lang_instruction = """
...
Target audience is international chefs and cooking enthusiasts interested in Japanese knives.
"""

    # システムプロンプト
    system_prompt = f"""You are an expert Japanese knife reviewer and culinary writer for Japanese Knife Archive (japaneseknifearchive.com).
You write authoritative, detailed, and engaging content about Japanese kitchen knives.
Your tone is knowledgeable but accessible — like a skilled chef explaining to an eager student.
{lang_instruction}
...
KNIVES DATA (first 10):
{json.dumps(knives_data[:10], indent=2, ensure_ascii=False)[:3000]}

STEELS DATA:
{json.dumps(steels_data, indent=2, ensure_ascii=False)[:2000]}
...
2. Article body:
   - 800-1500 words
   - Use ## for H2, ### for H3
   - Include specific knife recommendations with prices
   - Reference actual knives from the data
...
def main():
    # コマンドライン引数のパース
    parser = argparse.ArgumentParser(description="Japanese Knife Archive 記事自動生成スクリプト")
    parser.add_argument("--type", type=str, default="guide",
                        choices=["type", "steel", "budget", "use", "comparison", "brand", "guide"],
    ...
    knives_path = project_root / "src" / "data" / "knives.json"
    steels_path = project_root / "src" / "data" / "steels.json"
    brands_path = project_root / "src" / "data" / "brands.json"

    # データの読み込み
    knives_data = load_json(str(knives_path))
    steels_data = load_json(str(steels_path))
    ...
    print(f"=== Japanese Knife Archive 記事生成 ===")
    ...
    for i, keyword in enumerate(keywords_to_generate, 1):
        ...
        try:
            ...
            content = generate_article(
                ...
                knives_data, steels_data, brands_data
            )
